...
export default function BudgetConstraintPage() {
  const { t } = useI18n()
  const game = getGame('budget-constraint')!
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        <p className="text-slate-500 mt-2 mb-10 text-base">{t(game.descHu, game.descEn)}</p>

        <div className="flex flex-col gap-3">
          {game.levels.map((level, i) => (
            <Link key={level.id} to={`/games/budget-constraint/level/${level.id}`}
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${
                i === 0 ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-indigo-50 group-hover:text-indigo-600'
              }`}>
                {level.id}
              </div>
